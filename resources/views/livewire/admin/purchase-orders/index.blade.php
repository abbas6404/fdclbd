                ...
                <div class="card-header bg-white py-1">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="card-title mb-0 text-primary">
                            ...
                            {{ $editing_po_id ? 'Edit Purchase Order' : 'Create Purchase Order' }}
                        </h6>
                    </div>
                </div>
                ...
                <div class="col-md-7 px-0">
                    ...
                    <div class="card border">
                        <div class="card-header bg-light py-1">
                            <h6 class="mb-0"><i class="fas fa-file-alt me-1"></i> Purchase Order Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <div class="row">
                                        <label class="col-sm-4 col-form-label">PO Date<span class="text-danger">*</span></label>
                                        <div class="col-sm-8">
                                            <input type="date" 
                                                   id="purchase-order-date"
                                                   class="form-control form-control-sm @error('purchase_order_date') is-invalid @enderror" 
                                                   wire:model="purchase_order_date">
                                ...
                                <div class="col-md-6">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <input type="text" 
                                                       id="supplier-search" 
                                                       class="form-control form-control-sm {{ $selected_supplier_id ? 'bg-success bg-opacity-10 border-success' : '' }} @error('selected_supplier_id') is-invalid @enderror" 
                                                       wire:model.live.debounce.300ms="supplier_search" 
                                                       wire:click="showRecentSuppliers"
                                    ...
                                    <div class="row mt-2">
                                        <label class="col-sm-4 col-form-label">Requisition</label>
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <input type="text" 
                                                       id="requisition-search" 
                                                       class="form-control form-control-sm {{ $selected_requisition_id ? 'bg-success bg-opacity-10 border-success' : '' }}" 
                                                       wire:model.live.debounce.300ms="requisition_search" 
                                                       placeholder="Search requisition..." 
                                                       autocomplete="new-password">
                                                @if($selected_requisition_id)
                                                    <button type="button" 
                                                            class="btn btn-sm btn-outline-danger" 
                                                            wire:click="clearRequisition"
                                                            title="Clear Requisition">
                                                        <i class="fas fa-times"></i>
                                                    </button>
                                                @endif
                                            </div>
                                    ...
                                    <div class="row mt-2">
                                        ...
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <input type="text" 
                                                       id="employee-search" 
                                                       class="form-control form-control-sm {{ $selected_employee_id ? 'bg-success bg-opacity-10 border-success' : '' }}" 
                            ...
                            <div class="table-responsive">
                                ...
                                @foreach($items as $index => $item)
                                    ...
                                    <tfoot class="table-light">
                                        <tr>
                                            <th class="text-end" colspan="5">Total Amount:</th>
                ...
                <div class="col-md-5">
                    ...
                    <div class="card border mb-3" id="search-results-container">
                        <div class="card-header bg-primary text-white py-1">
                            <h6 class="mb-0">
                                ...
                                @elseif($active_search_type === 'employee')
                                    Recent Employees
                                @elseif($active_search_type === 'supplier')
                                    Recent Suppliers
                                @elseif($active_search_type === 'requisition')
                                    Requisitions
                        ...
                        <div class="card-body p-0" style="height: 400px; overflow-y: auto;" id="search-results-body">
                            ...
                            @elseif($active_search_type === 'supplier' && count($supplier_results) > 0)
                                <div class="table-responsive">
                                    <table class="table table-sm table-hover mb-0">
                                        <thead class="table-light sticky-top">
                                            <tr>
                                        ...
                                        <tbody>
                                @foreach($supplier_results as $result)
                                            <tr class="search-item" 
                                     wire:click="selectSupplier({{ $result['id'] }})"
                                     style="cursor: pointer;">
                                                <td class="small text-nowrap arrow-indicator" title="{{ $result['name'] ?? 'N/A' }}">
                                                    <span class="arrow-icon">▶</span>
                                                    <strong>{{ $result['name'] ?? 'N/A' }}</strong>
                            ...
                            @elseif($active_search_type === 'requisition' && count($requisition_results) > 0)
                                <div class="table-responsive">
                                    <table class="table table-sm table-hover mb-0">
                                        <thead class="table-light sticky-top">
                                            <tr>
                                ...
                                @foreach($requisition_results as $result)
                                            <tr class="search-item" 
                                     wire:click="selectRequisition({{ $result['id'] }})"
                                     style="cursor: pointer;">
                                                <td class="small text-nowrap arrow-indicator" title="{{ $result['requisition_number'] ?? 'N/A' }}">
                                                    <span class="arrow-icon">▶</span>
                                                    <strong>{{ $result['requisition_number'] ?? 'N/A' }}</strong>
                                                </td>
                                                <td class="small text-nowrap">
                                                    {{ $result['requisition_date'] ?? 'N/A' }}
                                                </td>
                                                <td class="small text-nowrap">
                                                    {{ Str::limit($result['employee_name'] ?? 'N/A', 20) }}
                                                </td>
                                                <td class="small text-nowrap">
                            ...
                            @else
                                <div class="p-3 text-center text-muted">
                                    ...
                                    @elseif($active_search_type === 'supplier')
                                        <p>No suppliers found. Start searching to see results.</p>
                                    @elseif($active_search_type === 'requisition')
                                        <p>No requisitions found. Start searching to see results.</p>
                                    @elseif($active_search_type === 'account')
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            document.addEventListener('DOMContentLoaded', function() {
                const poDateInput = document.getElementById('purchase-order-date');
