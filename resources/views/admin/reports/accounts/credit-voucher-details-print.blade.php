...
<head>
    ...
    <title>Credit Voucher Details Report - {{ \Carbon\Carbon::parse($dates['start'])->format('M d, Y') }} to {{ \Carbon\Carbon::parse($dates['end'])->format('M d, Y') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <div class="report-header">
            <div class="company-info">
                ...
                <div class="report-info">
                    <h2>Credit Voucher Details Report</h2>
            ...
            <tbody>
                @forelse($vouchers as $voucher)
                    ...
                    <tr>
                        ...
                        <td class="text-left">{{ $item->headOfAccount->account_name ?? '-' }}</td>
                        <td class="text-left">{{ $item->description ?? '-' }}</td>
                    </tr>
                    @endforeach
                @empty
                <tr>
                    <td colspan="5" class="text-center">No credit voucher items found</td>
                </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr style="background-color: #e9ecef; font-weight: bold;">
                    ...
                    <th class="text-right">
                        {{ number_format($vouchers->sum(function($v) { return $v->items->sum('amount'); }), 0) }}
                    </th>
                    <th colspan="2"></th>
                </tr>
            </tfoot>
