...
<head>
    ...
    <title>Contra Entry Report - {{ \Carbon\Carbon::parse($dates['start'])->format('M d, Y') }} to {{ \Carbon\Carbon::parse($dates['end'])->format('M d, Y') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right"><strong>{{ number_format($entry->items->where('entry_type', 'debit')->sum('amount'), 0) }}</strong></td>
                    <td class="text-right"><strong>{{ number_format($entry->items->where('entry_type', 'credit')->sum('amount'), 0) }}</strong></td>
                    <td class="text-left">{{ $entry->remarks ?? '-' }}</td>
                    <td>{{ $entry->created_at->format('d M Y') }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="text-center">No contra entries found</td>
                </tr>
                @endforelse
            </tbody>
            <tfoot>
