...
<head>
    ...
    <title>Journal Entry Details Report - {{ \Carbon\Carbon::parse($dates['start'])->format('M d, Y') }} to {{ \Carbon\Carbon::parse($dates['end'])->format('M d, Y') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <div class="report-header">
            <div class="company-info">
                ...
                <div class="report-info">
                    <h2>Journal Entry Details Report</h2>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th style="width: 27%;">Head of Account</th>
            ...
            <tbody>
                @forelse($entries as $entry)
                    ...
                    <tr>
                        ...
                        <td class="text-right"><strong>{{ number_format($debit->amount, 0) }}</strong></td>
                        <td class="text-right">-</td>
                        <td class="text-left">{{ $debit->headOfAccount->account_name ?? '-' }}</td>
                        <td class="text-left">{{ $debit->description ?? '-' }}</td>
                    </tr>
                    @endforeach
                    ...
                    <tr>
                        ...
                        <td>{{ $entry->entry_date->format('d M Y') }}</td>
                        <td class="text-right">-</td>
                        <td class="text-right"><strong>{{ number_format($credit->amount, 0) }}</strong></td>
                        <td class="text-left">{{ $credit->headOfAccount->account_name ?? '-' }}</td>
                        <td class="text-left">{{ $credit->description ?? '-' }}</td>
                    </tr>
                    @endforeach
                @empty
                <tr>
                    <td colspan="6" class="text-center">No journal entry items found</td>
                </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr style="background-color: #e9ecef; font-weight: bold;">
                    ...
                    <th class="text-right">
                        {{ number_format($entries->sum(function($e) { return $e->debits->sum('amount'); }), 0) }}
                    </th>
                    <th class="text-right">
                        {{ number_format($entries->sum(function($e) { return $e->credits->sum('amount'); }), 0) }}
                    </th>
                    <th colspan="2"></th>
                </tr>
            </tfoot>
