...
<head>
    ...
    <title>Credit Voucher Report - {{ \Carbon\Carbon::parse($dates['start'])->format('M d, Y') }} to {{ \Carbon\Carbon::parse($dates['end'])->format('M d, Y') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th style="width: 20%;">Treasury Account</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-left">{{ $voucher->treasuryAccount->account_name ?? '-' }}</td>
                    <td class="text-right"><strong>{{ number_format($voucher->total_amount, 0) }}</strong></td>
                    <td class="text-left">{{ $voucher->remarks ?? '-' }}</td>
                    <td>{{ $voucher->created_at->format('d M Y') }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="text-center">No credit vouchers found</td>
                </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr style="background-color: #e9ecef; font-weight: bold;">
                    ...
                    <th class="text-right">
                        {{ number_format($vouchers->sum('total_amount'), 0) }}
                    </th>
                    <th colspan="2"></th>
                </tr>
            </tfoot>
